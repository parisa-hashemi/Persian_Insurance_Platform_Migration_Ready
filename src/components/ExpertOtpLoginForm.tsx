...
export const ExpertOtpLoginForm: React.FC<ExpertOtpLoginFormProps> = ({
  ...
  // When company changes or on mount, if staff exists, populate first one as suggestion
  useEffect(() => {
  ...
  // Handle staff selection from quick chips or dropdown
  const handleSelectPredefinedStaff = (staff: StaffMember) => {
  ...
  const handleSendOtp = (e?: React.FormEvent) => {
    ...
    if (!cleanName) {
      setErrorMsg('لطفاً نام و نام خانوادگی کارشناس را وارد نمایید.');
      return;
    }
  ...
  return (
    <div className="space-y-4 animate-in fade-in">
      ...
      {/* STEP 1: SELECT COMPANY, ENTER NAME & PHONE */}
      {step === 'INFO' ? (
        <form onSubmit={handleSendOtp} className="space-y-4">
          {/* 1. Insurance Company Selector */}
          <div>
            ...
            <select
              ...
              className="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs text-slate-900 font-bold focus:outline-none focus:border-blue-900 shadow-sm cursor-pointer"
            >
              {insurersList.map((c) => (
                <option key={c.code} value={c.code}>
                  {c.name} {c.adminName ? `— (${c.province || 'شعبه مرکزی'})` : ''}
                </option>
              ))}
            </select>
          </div>

          {/* Quick Staff Selection Chips if Available */}
          {companyStaff.length > 0 && (
            <div className="bg-slate-100/80 p-2.5 rounded-xl border border-slate-200 space-y-1.5">
              <span className="text-[11px] text-slate-600 font-bold flex items-center gap-1">
                ...
                پرسنل ثبت‌شده در شرکت {currentCompany?.name}:
              </span>
              <div className="flex flex-wrap gap-1.5">
                {companyStaff.map((staff) => (
                  <button
                    key={staff.id}
                    type="button"
                    onClick={() => handleSelectPredefinedStaff(staff)}
                  ...
                  >
                    ...
                    {staff.phone && <span className="font-mono text-[10px] opacity-75">({staff.phone})</span>}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div>
            <label className="block text-xs text-slate-800 mb-1.5 font-bold flex items-center gap-1.5">
              ...
              <span>نام و نام خانوادگی کارشناس</span>
            </label>
            <input
              type="text"
              value={expertName}
              ...
              className="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs text-slate-900 font-bold focus:outline-none focus:border-blue-900 shadow-sm transition"
              required
            />
          </div>
          ...
          <div>
            <label className="block text-xs text-slate-800 mb-1.5 font-bold flex items-center justify-between">
              ...
              <span className="text-[10px] text-slate-500 font-mono">۰۹xxxxxxxx</span>
            </label>
            <input
              type="tel"
              value={expertPhone}
              ...
              placeholder="09121001001"
              className="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs font-mono text-slate-900 font-bold focus:outline-none focus:border-blue-900 shadow-sm transition"
              dir="ltr"
              required
            />
          </div>
          ...
          <button
            type="submit"
            className={`w-full py-3.5 font-black rounded-xl text-xs shadow-md transition-all flex items-center justify-center gap-2 active:scale-95 ${roleConfig.btnBg}`}
          >
        ...
        <form onSubmit={handleVerifyOtp} className="space-y-4 animate-in fade-in">
            ...
            <div className="bg-amber-50/90 border-2 border-amber-300 p-3.5 rounded-2xl space-y-2 shadow-sm">
              ...
              <div className="pt-1 flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-2.5 py-1 bg-amber-500 hover:bg-amber-400 text-blue-950 rounded-lg text-xs font-black shadow-sm transition active:scale-95"
                >
                  درج خودکار کد
                </button>
              </div>
          ...
          <div className="bg-white p-3 rounded-xl border border-slate-200 text-xs flex items-center justify-between">
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-lg text-[11px] font-bold text-slate-600 hover:text-blue-900 hover:bg-slate-100 transition flex items-center gap-1"
            >
