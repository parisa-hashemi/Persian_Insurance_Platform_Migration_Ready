...
import { MaskedCloudCallModal } from '../common/MaskedCloudCallModal';
...
export const CustomerExpertCallModal: React.FC<CustomerExpertCallModalProps> = ({
  ...
  const [showCloudCall, setShowCloudCall] = useState(false);
  ...
  const expertPhoneMasked = '۰۲۱-۹۱۰۹۸۸۰۰ (داخلی ابری)';
    ...
    <>
      <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-start sm:items-center justify-center p-3 sm:p-4 overflow-y-auto" dir="rtl">
        <div className="bg-white rounded-3xl p-4 sm:p-6 sm:p-7 max-w-md w-full shadow-2xl border-2 border-slate-200 space-y-5 animate-in zoom-in-95">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-extrabold text-base text-slate-900">
                  تماس امن ابری با کارشناس ارزیاب
                </h3>
          ...
          {/* Expert Info Card with Masked Cloud Call */}
          <div className="bg-gradient-to-br from-indigo-50 via-blue-50 to-white border-2 border-indigo-200 rounded-2xl p-4 space-y-3">
            ...
            <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 space-y-1 text-xs">
              <div className="flex items-center gap-1.5 font-bold text-amber-900">
                ...
                <span>پنهان‌سازی شماره و تماس ابری ضدتبانی:</span>
              </div>
              <p className="text-[11px] text-amber-950/80 leading-relaxed font-medium">
                جهت حفظ حریم خصوصی و جلوگیری از هرگونه تبانی، شماره تلفن مستقیم طرفین پنهان بوده و تماس از طریق درگاه ابری واسط و ضبط نظارتی هوشمند برقرار می‌گردد.
              </p>
            </div>

            <div className="p-3 bg-white rounded-xl border border-indigo-100 space-y-1.5 text-xs">
              <div className="flex items-center justify-between">
                <span className="text-slate-500 font-bold">مسیر ارتباط تلفنی:</span>
                <span className="font-mono font-bold text-indigo-700 text-xs" dir="ltr">{expertPhoneMasked}</span>
              </div>
              <div className="flex items-center justify-between">
            ...
            {/* Masked Cloud VoIP Button */}
            <button
              type="button"
              onClick={() => setShowCloudCall(true)}
            ...
            >
              ...
              <span>برقراری تماس امن ابری با کارشناس (بدون نمایش شماره)</span>
            </button>
          </div>
      ...
      {/* Masked Cloud Call VoIP Modal */}
      {showCloudCall && (
        <MaskedCloudCallModal
          isOpen={showCloudCall}
          onClose={() => setShowCloudCall(false)}
          callerName={session.name || claimCase.victimName || 'بیمه‌گذار / زیان‌دیده'}
          callerRole="زیان‌دیده حادثه"
          callerPhoneMasked="۰۹۱۲***۷۷۶۵"
          calleeName={expertName}
          calleeRole={`کارشناس ارزیاب (${expertRole})`}
          calleePhoneMasked="۰۹۱۹***۱۱۴۳"
          caseId={claimCase.id}
        />
      )}
    </>
